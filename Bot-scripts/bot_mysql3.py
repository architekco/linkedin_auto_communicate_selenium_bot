...
search_keyword_query = """SELECT * FROM connector_search 
WHERE id='%s' order by searchdate desc limit 1"""
...
def get_task_row(cur, status=botstatus.QUEUED):
    cur.execute("SELECT * FROM app_bottask where status ='%s' order by id desc limit 1" % (status))
    row = cur.fetchone()    
    return row
...
# cur, owner_id, task_id, task_type
...
def handle_login(cur, owner_id, task_id, task_type):
    ...
    bot_result = login_linkedin(email, password, None, cur, owner_id, task_id, task_type)
    if(len(bot_result) == 4):
        return
...
def handle_pinverify(cur, owner_id, task_id, task_type, taskrow):
    ...
    status, lastrun_date, completed_date = login_linkedin(email, password, 
        pindata['pin'], cur, owner_id, task_id, task_type)
...
def get_messages(email, password,  cur, owner_id):
    ...
    try:
        ...
        for messaging in messaging_list:
            ...
            messaging_text_lis = messaging_text_div.find_elements_by_tag_name("p")

            for messaging_text_li in messaging_text_lis:
                messaging_text = messaging_text_li.text
                text = messaging_text.encode('utf-8')
                print('******************')
                print(text)
                
            ## add to db
            #add_to_db(search_query, )
            #cur.execute(getmessages_query, (text, time, contact_id))
        ...
        # driver.close()
        #return created_at_time, update_at, text, datetime.now(), replied_date, replied_other_date, contact_id, bot_status, lastrun_date, completed_date
